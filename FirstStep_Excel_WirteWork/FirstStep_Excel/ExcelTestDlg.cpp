...
void CExcelTestDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_MFCEDITBROWSE2, m_FileOut2);
	DDX_Text(pDX, IDC_MFCEDITBROWSE3, m_FileIn);
}
...
//	ON_EN_CHANGE(IDC_MFCEDITBROWSE3, &CExcelTestDlg::OnChangeMfceditbrowse3)
//	ON_EN_UPDATE(IDC_MFCEDITBROWSE3, &CExcelTestDlg::OnUpdateMfceditbrowse3)
END_MESSAGE_MAP()
...
//--------------------------чтение----------------------------------
	...
	for (int rowCounter = 1; rowCounter <= 171; rowCounter++) {
			...
			for (int rowCounter2 = 1; rowCounter2 <= iRows2; rowCounter2++) {
				...
				CString w = CString(vData) + _T(" - счет ненайден в файл-приемнике");
				AfxMessageBox(_T(w));
			}
			}
